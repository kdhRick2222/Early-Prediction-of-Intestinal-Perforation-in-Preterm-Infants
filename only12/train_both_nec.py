...
if __name__ == '__main__':
    ...
    parser.add_argument('--outputs-dir', type=str, default="/home/daehyun/KNN/only12/sgd_weight")
    ...
    train_dataset = TrainDataset_nec(args.data)
    ...
    for epoch in range(args.num_epochs):
        ...
        ntety_loss = (ntety_loss1 + ntety_loss2) /(n1 + n2)
        ...
        # binarizer = Binarizer(threshold=best_threshold).fit(predict1)
        # custom_predict = binarizer.transform(predict1)
        ...
        print("EPOCH : {0:3d}  ntet_loss : {1:0.4f}  AUC_ntet : {2:0.4f}  AUC_ntety : {3:0.4f}, ppv(ntet) : {4:0.4f}, npv(ntet) : {5:0.4f}, sensitivity : {6:0.4f}"
        .format(epoch, ntet_loss, auc_ntet, auc_ntety, ppv, npv, sensitivity))
        
        if epoch > 200:
            early_stopping(ntet_loss, model)
            
            if min_loss > ntet_loss:
                min_loss = ntet_loss
